...
int iscnan(double complex d) {
  return !isinf(creal(d)) && !isinf(cimag(d)) &&
         (isnan(creal(d)) || isnan(cimag(d)));
}
    ...
    for (j = 0; j < nelem(f); j++)
      for (k = 0; k < nelem(f); k++)
        for (l = 0; l < nelem(f); l++) {
          ...
          // results should agree in kind.  Override the gcc computation in this
          // case.
          if (iscnan(n) && d == 0)
            q = (NAN + NAN * I) / zero;
